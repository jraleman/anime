...
<div
  class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 py-8 mx-auto max-w-screen-xl lg:py-16 justify-items-center"
>
  {#each $animeList as anime}
    <div class="card bg-neutral w-72 shadow-xl">
      <div
        class="full-width rounded-t-xl text-center p-1 text-gray-900 bg-{getAnimeStatusColor(anime.list_status.status)}"
      >
        {getAnimeStatusLabel(anime.list_status.status)}
      </div>
      <figure>
        <img class="size-full" src={anime.node.main_picture.large} alt={anime.node.title} />
      </figure>
      <div class="card-body">
        <h2 class="card-title">
          {anime.node.title}
        ...
        <div class="modal" role="dialog">
          <div class="modal-box">
            ...
            <div class="modal-action">
              <label for={anime.node.id} class="btn btn-secondary">Close</label>
            </div>
          </div>
        </div>
      </div>
